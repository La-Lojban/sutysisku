...
const worker = new Worker('worker.js?sisku=1642520755265')
initBackend(worker)
      ...
      <div class='DIV_1' style='height:${height}px;width:${width * cisn_default
        ...
        <div class='DIV_2' style='height:${height}px;width:${width * cisn_default
          ...
          <a${(url || '').indexOf('http') === 0
            ...
            : ''
          } aria-label="${title.replace(/<[^>]+?>/g, '')}" onclick="window.ningau_lepasorcu('${url}','${bangu}')" href="${url}" class='A_7'>
          <div class='DIV_8' style='height:${height}px;width:${width * cisn_default
          }px;background-image:url("${picture}")'>
          </div>
          </a>
        </div>
      </div>`
    ...
    if (fullDef.bangu.indexOf('-cll') >= 0) {
      const d = Object.keys(def)
        .map((address) => {
          ...
          return `<li><a rel='noreferrer' target='_blank' href="${window.leijufra.uncll_url}${address}">${velcki}</a></li>`
        })
        .join('')
      return {
